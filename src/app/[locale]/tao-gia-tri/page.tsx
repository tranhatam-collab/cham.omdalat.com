...
export default async function CreateValueViPage({ params }: Props) {
  const { locale: raw } = await params;
  const locale = (raw === "en" ? "en" : "vi") as Locale;
  const dict = getDictionary(locale);
  const isVi = locale === "vi";
  const step = dict.journey.fourSteps.step3;
    ...
    <div className="max-w-4xl mx-auto px-4 py-16">
      ...
      <div className="p-8 rounded-[--radius-om] border border-[rgba(21,49,38,0.12)] bg-[rgba(255,255,255,0.78)] backdrop-blur-sm mb-8">
        <p className="text-[--color-om-text] leading-relaxed mb-6">
          {isVi
            ? "Từ thử nghiệm thành công, bạn xây dựng những bằng chứng có thể trở thành giá trị thực cho người khác — một portfolio, một dịch vụ, một project."
            : "From successful experiments, you build evidence that can become real value for others — a portfolio, a service, a project."}
        </p>
      </div>

      <h2 className="font-[family-name:var(--font-heading)] text-2xl font-bold text-[--color-om-green-700] mb-6">
        {isVi ? "Cách tạo giá trị" : "Ways to Create Value"}
      </h2>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center">
        <Link
          href={isVi ? `/${locale}/trao-lai` : `/${locale}/give-back`}
        ...
        >
          {isVi ? "Bước tiếp: Trao lại" : "Next: Give Back"}
